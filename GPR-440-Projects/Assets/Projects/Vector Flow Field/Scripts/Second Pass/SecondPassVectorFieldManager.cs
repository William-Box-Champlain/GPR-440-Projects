...
namespace VFF
{
    ...
    /// </summary>
    public class SecondPassVectorFieldManager : MonoBehaviour
    {
        private static SecondPassVectorFieldManager _instance;

        public static SecondPassVectorFieldManager GetInstance()
        {
            if (_instance == null) _instance = new SecondPassVectorFieldManager();
            return _instance;
        }
        ...
        public void RegisterSource(VectorFieldSource source)
        {
            if (source != null && !sources.Contains(source))
            {
                ...
                previousSourcePositions[source] = source.gameObject.transform.position;
                UpdateSinksAndSources();
            }
        }
        ...
        public void UpdateSinksAndSources()
        {
            ...
            foreach (VectorFieldSource source in sources)
            {
                ...
                {
                    Vector2 normalizedPosition = WorldToNormalizedPosition(source.gameObject.transform.position);
                    Bounds bounds = WorldBounds;
